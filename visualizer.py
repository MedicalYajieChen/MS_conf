...
def save_images(visuals, seg_path, image_path, target_path, aspect_ratio=1.0, width=256, direction="BtoA"):
    ...
    short_path_target = ntpath.basename(target_path[0])
    name_target = os.path.splitext(short_path_target)[0]
    ...
    for label, im_data in visuals.items():
        ...
        if direction == "BtoA":
            if label == 'fake_A':
                ...
                new_name = name.replace('image', 'label')
                image_name = '%s.png' % new_name
                vis_dict['label'] = im
    ...
    # plt.subplot(121)
    # plt.imshow(vis_dict['img'])
    # plt.title('image')
    # plt.subplot(122)
    # plt.imshow(vis_dict['label'])
    # plt.title('label')
    # plt.savefig(os.path.join(seg_path, '%s.jpg'))
    # plt.close()
...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, save_result):
        ...
        #             # images.append(image_numpy.transpose([1, 2, 0]))
        #             images.append(image_numpy)
        #             idx += 1
        #             if idx % ncols == 0:
        #                 label_html += '<tr>%s</tr>' % label_html_row
        #                 label_html_row = ''
        #         # white_image = np.ones_like(image_numpy.transpose([1, 2, 0])) * 255
