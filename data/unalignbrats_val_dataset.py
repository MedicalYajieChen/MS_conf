...
class UnalignBratsValDataset(BaseDataset):
    def __init__(self, opt):
        BaseDataset.__init__(self, opt)
        self.opt = opt

        self.source_dir = os.path.join(opt.dataroot, "t2_test_seg_data")
        self.target_dir = os.path.join(opt.dataroot, 't1_test_seg_data') if opt.target == "t1" \
            ...
            else os.path.join(opt.dataroot, "flair_test_seg_data")
        self.isTrain = False
        ...
        self.vis_path = 'brats_vis/t2flair/' + opt.data_input + '_' + opt.name + '_' + opt.epoch
