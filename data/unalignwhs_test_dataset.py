...
class UnalignWHSTestDataset(BaseDataset):
    def __init__(self, opt):
        ...
        self.target_dir = os.path.join(opt.dataroot, 'ct_'+opt.data_input+'_seg_data') if opt.direction == 'BtoA'\
            else os.path.join(opt.dataroot, 'mr_'+opt.data_input+'_seg_data')
        ...
        self.vis_path = 'whs_vis/ct2mr/'+opt.data_input+'_'+opt.name+'_new_' + opt.epoch if \
            self.opt.direction == 'AtoB' else 'whs_vis/mr2ct/'+opt.data_input+'_'+opt.name+'_new_'  + opt.epoch
    ...
    def __getitem__(self, index):
        ...
        tgt_lb_path = target_path.replace('image', 'gth').replace('.png', '.pgm')
