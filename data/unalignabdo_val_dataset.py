...
class UnalignAbdoValDataset(BaseDataset):
    def __init__(self, opt):
        ...
        self.source_dir = os.path.join(opt.dataroot, "t2_test_seg_data") if opt.direction == "BtoA" \
            else os.path.join(opt.dataroot, "ct_test_seg_data")
        self.target_dir = os.path.join(opt.dataroot, 'ct_test_seg_data') if opt.direction == "BtoA" \
            else os.path.join(opt.dataroot, "t2_test_seg_data")
        self.isTrain = False
        self.vis_path = 'abdo_vis/t22ct/' + opt.data_input + '_' + opt.name + '_' + opt.select_epoch \
            if opt.direction == "BtoA" else 'abdo_vis/ct2t2/' + opt.data_input + '_' + opt.name + '_' + opt.select_epoch
        ...
        self.target_paths = sorted(self.make_dataset(self.target_dir), key = lambda i:int(i.split('/')[-1].\
                                                                                          split('_')[0].split('img')[1]))
